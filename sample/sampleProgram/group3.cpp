...
Group3::Weights Group3::Weights::fromEnv(const char *prefix) {
    ...
    w.pass        = get("W_PASS", w.pass);
    w.weak        = get("W_WEAK", w.weak);
    w.joker       = get("W_JOKER", w.joker);
    w.pair        = get("W_PAIR", w.pair);
    w.danger      = get("W_DANGER", w.danger);
    w.minOpp      = get("W_MINOPP", w.minOpp);
    w.endgame     = get("W_ENDGAME", w.endgame);
    w.endgameDeep = get("W_ENDGAME_DEEP", w.endgameDeep);
    w.leaderWeak  = get("W_LEADER_WEAK", w.leaderWeak);
    w.multiLead   = get("W_MULTI_LEAD", w.multiLead);
    w.oneMore     = get("W_ONE_MORE", w.oneMore);
    return w;
}
...
double Group3::scoreMove(const CardSet &move, const GameStatus &gstat) const {
    ...
    int hsize = hand.size();
    // pass重みは手札数に応じて動的に増加（基本値 w.pass）
    double passWeight = w.pass;
    if (hsize <= 5) passWeight = w.pass * 2.0;
    if (hsize <= 3) passWeight = w.pass * 3.0;
    if (hsize <= 1) passWeight = w.pass * 4.0;

    // ジョーカー温存ペナルティも終盤は弱める
    double jokerPenalty = w.joker;
    if (hsize <= 4) jokerPenalty = w.joker * 0.5;
    if (hsize <= 2) jokerPenalty = 0.0;

    double score = 0.0;
    score += passWeight * pass;
    score += (15 - strength) * w.weak;
    score -= useJoker ? jokerPenalty : 0.0;
    // ペア温存（手札が多いうちは強めに、終盤は弱めに）
    double pairWeight = w.pair;
    if (hsize <= 4) pairWeight = w.pair * 0.5;
    if (hsize <= 2) pairWeight = w.pair * 0.125;
    score -= pairBroken * pairWeight;

    // ライバル接近時の止め強化（人数で重みを倍化）
    if (dangerCount > 0) score += dangerCount * w.danger * pass;
    if (minOpp <= 1) score += w.minOpp;  // 誰かリーチなら出すこと自体に価値

    // 終盤は積極的に出す（パスのベースライン 0 を超えやすくする）
    if (hsize <= 5) score += w.endgame;
    if (hsize <= 3) score += w.endgameDeep;  // 追加ボーナス

    if (gstat.pile.size() == 0) {
        score += (15 - strength) * w.leaderWeak;
        // 複数枚リード（ペア・トリプルなど）は相手の選択肢を狭めるのでボーナス
        if ((int)move.size() >= 2) score += w.multiLead;
        if ((int)move.size() >= 3) score += w.multiLead;
    }
