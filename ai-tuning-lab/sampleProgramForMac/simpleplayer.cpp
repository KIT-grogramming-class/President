...
const int kLeadCardCountWeight = 700;
const int kLeadGroupBonus = 520;
const int kFollowBeatBonus = 2620;
const int kFollowCardCountWeight = 250;
const int kFollowStrengthPenalty = 75;
const int kWeakCardBonus = 75;
const int kShapeWeight = 1;
const int kBreakPairPenalty = 440;
const int kFinishTurnPenalty = 1090;
const int kEndgameCardBonus = 1070;
const int kNearEndgameCardBonus = 210;
const int kDangerTakeLeadBonus = 650;
const int kPassBaseScore = 0;
const int kPassConserveBonus = 240;
const int kPassEndgamePenalty = 1450;
const int kPassNearEndgamePenalty = 440;
const int kPassDangerPenalty = 530;
const int kMcCandidateLimit = 3;
const int kMcPlayouts = 6;
const int kMcThreads = 2;
const int kMcMaxPlies = 45;
const int kMcWinRateWeight = 450;
const int kMcFinishEaseWeight = 80;
const int kMcBaseScoreDivisor = 1;
const int kMcOverrideWindow = 1200;
const int kMcOverrideThreshold = 350;
...
bool sameMove(const CandidateMove &a, const CandidateMove &b) {
    if (a.pass != b.pass) {
        return false;
    }
    if (a.pass) {
    ...
    for (int i = 0; i < a.cards.size(); i++) {
        if (!b.cards.member(a.cards.at(i))) {
            return false;
        }
    }
    return true;
...
int playerIndexOf(const GameStatus &gstat, int playerId) {
    for (int i = 0; i < gstat.numPlayers; i++) {
        if (gstat.playerID[i] == playerId) {
            return i;
        }
    }
    return gstat.turnIndex;
}
...
std::vector<ScoredMove> topScoredMoves(
    const std::vector<CandidateMove> &moves,
    const CardSet &hand,
    ...
    for (size_t i = 0; i < moves.size(); i++) {
        ScoredMove item;
        item.move = moves[i];
        item.score = scoreMove(moves[i], hand, gstat, myId);
    ...
    std::sort(scored.begin(), scored.end(), [](const ScoredMove &a, const ScoredMove &b) {
        ...
        if (a.move.pass != b.move.pass) {
            return !a.move.pass;
        }
        return effectiveStrength(a.move.cards) < effectiveStrength(b.move.cards);
    });

    std::vector<ScoredMove> top;
    for (size_t i = 0; i < scored.size() && static_cast<int>(top.size()) < limit; i++) {
        bool duplicate = false;
        for (size_t j = 0; j < top.size(); j++) {
            if (sameMove(scored[i].move, top[j].move)) {
                duplicate = true;
                break;
            }
        }
        if (!duplicate) {
            top.push_back(scored[i]);
        }
    }
    return top;
}

GameStatus makeSimStatus(const GameStatus &base, const SimState &state) {
    GameStatus ret = base;
    ret.pile = state.pile;
    ret.turnIndex = state.turn;
    ret.leaderIndex = state.leader;
    ret.numPlayers = static_cast<int>(state.hands.size());
    for (int i = 0; i < ret.numPlayers; i++) {
        ret.numCards[i] = state.hands[i].size();
    }
    return ret;
}

std::mt19937 &monteCarloRng() {
    static std::mt19937 rng(20260501);
    return rng;
}

std::vector<Card> fullDeckCards() {
    std::vector<Card> deck;
    for (int suit = Card::SUIT_SPADE; suit <= Card::SUIT_CLUB; suit++) {
        for (int rank = 1; rank <= 13; rank++) {
            deck.push_back(Card(suit, rank));
        }
    }
    deck.push_back(Card(Card::SUIT_JOKER, Card::RANK_JOKER));
    return deck;
}

std::vector<Card> unknownCards(const CardSet &myHand, const CardSet &seenCards, const CardSet &pile) {
    CardSet known(myHand);
    known.insert(seenCards);
    known.insert(pile);

    std::vector<Card> unknown;
    std::vector<Card> deck = fullDeckCards();
    for (size_t i = 0; i < deck.size(); i++) {
        if (!known.includes(deck[i])) {
            unknown.push_back(deck[i]);
        }
    }
    return unknown;
}

void dealRandomOpponentHands(
    SimState &state,
    const GameStatus &gstat,
    const std::vector<Card> &unknown,
    int myIndex,
    std::mt19937 &rng
) {
    std::vector<Card> shuffled = unknown;
    ...
    for (int i = 0; i < gstat.numPlayers; i++) {
        ...
        const int need = gstat.numCards[i];
        for (int c = 0; c < need && pos < shuffled.size(); c++, pos++) {
            state.hands[i].insert(shuffled[pos]);
        }
    }
}

CandidateMove chooseSimulationMove(
    const CardSet &hand,
    const GameStatus &gstat,
    int playerId,
    std::mt19937 &rng
    ...
    std::vector<ScoredMove> top = topScoredMoves(moves, hand, gstat, playerId, 3);
    if (top.empty()) {
        CandidateMove passMove;
        passMove.pass = true;
        return passMove;
    ...
    // プレイアウトでは完全に貪欲にせず、上位手を少しランダムに混ぜて読みの偏りを減らす。
    static const int weights[] = { 6, 3, 1 };
    int totalWeight = 0;
    for (size_t i = 0; i < top.size(); i++) {
        totalWeight += weights[std::min(static_cast<int>(i), 2)];
    }
    std::uniform_int_distribution<int> dist(1, totalWeight);
    int ticket = dist(rng);
    for (size_t i = 0; i < top.size(); i++) {
        ticket -= weights[std::min(static_cast<int>(i), 2)];
        if (ticket <= 0) {
            return top[i].move;
        }
    }
...
int simulatePlayout(
    const CandidateMove &firstMove,
    const CardSet &hand,
    const CardSet &seenCards,
    const GameStatus &gstat,
    int myId,
    std::mt19937 &rng
) {
    const int myIndex = playerIndexOf(gstat, myId);
    SimState state;
    state.hands.assign(gstat.numPlayers, CardSet());
    state.hands[myIndex] = hand;
    state.pile = gstat.pile;
    state.turn = (gstat.turnIndex + 1) % gstat.numPlayers;
    state.leader = gstat.leaderIndex;
    state.passes = 0;

    if (!firstMove.pass) {
        state.hands[myIndex].remove(firstMove.cards);
        if (state.hands[myIndex].isEmpty()) {
            return 1;
        }
        state.pile = firstMove.cards;
        state.leader = myIndex;
    } else {
        state.passes = 1;
    }

    std::vector<Card> unknown = unknownCards(hand, seenCards, gstat.pile);
    dealRandomOpponentHands(state, gstat, unknown, myIndex, rng);

    for (int ply = 0; ply < kMcMaxPlies; ply++) {
        if (!state.pile.isEmpty() && state.passes > 0 && state.turn == state.leader) {
            state.pile.clear();
            state.passes = 0;
        }

        GameStatus simStatus = makeSimStatus(gstat, state);
        const int playerId = gstat.playerID[state.turn];
        CandidateMove move = chooseSimulationMove(state.hands[state.turn], simStatus, playerId, rng);

        if (move.pass || move.cards.isEmpty()) {
            state.passes++;
        } else {
            state.hands[state.turn].remove(move.cards);
            if (state.hands[state.turn].isEmpty()) {
                return state.turn == myIndex ? 1 : 0;
            }
            state.pile = move.cards;
            state.leader = state.turn;
            state.passes = 0;
        }

        state.turn = (state.turn + 1) % gstat.numPlayers;
    }

    const int myRemaining = state.hands[myIndex].size();
    int bestOpponent = 99;
    for (int i = 0; i < state.hands.size(); i++) {
        if (i != myIndex) {
            bestOpponent = std::min(bestOpponent, state.hands[i].size());
        }
    }
    return myRemaining <= bestOpponent ? 1 : 0;
}

int simulatePlayoutBatch(
    ...
    for (int i = 0; i < playouts; i++) {
        wins += simulatePlayout(move, hand, seenCards, gstat, myId, rng);
    }
    return wins;
}
...
int monteCarloScore(
    ...
    if (move.pass && gstat.pile.isEmpty()) {
    ...
    std::mt19937 &seedRng = monteCarloRng();
    int threadCount = std::min(kMcThreads, kMcPlayouts);
    const unsigned int hardwareThreads = std::thread::hardware_concurrency();
    if (hardwareThreads > 0) {
        threadCount = std::min(threadCount, static_cast<int>(hardwareThreads));
    }

    int wins = 0;
    if (threadCount <= 1) {
        wins = simulatePlayoutBatch(move, hand, seenCards, gstat, myId, kMcPlayouts, seedRng());
    } else {
        std::vector<std::future<int> > futures;
        int remainingPlayouts = kMcPlayouts;
        for (int t = 0; t < threadCount; t++) {
            const int batchSize = remainingPlayouts / (threadCount - t);
            remainingPlayouts -= batchSize;
            futures.push_back(std::async(
                std::launch::async,
                simulatePlayoutBatch,
                move,
                hand,
                seenCards,
                gstat,
    ...
    const FinishPlan finishPlan = analyzeFinishPlan(remaining);
    int finishEase = std::max(0, 5 - finishPlan.turns) * kMcFinishEaseWeight;
    if (canPlayInOneMove(remaining)) {
        finishEase += 1800;
    }

    return wins * kMcWinRateWeight / kMcPlayouts + finishEase;
}

bool shouldUseMonteCarlo(const CardSet &hand, const GameStatus &gstat, int myId) {
    return hand.size() <= 6 || minimumOpponentCards(gstat, myId) <= 3;
}

CandidateMove chooseBestMove(
    const CardSet &hand,
    const CardSet &seenCards,
    const GameStatus &gstat,
    int myId
    ...
    if (!shouldUseMonteCarlo(hand, gstat, myId)) {
        return best;
    }

    std::vector<ScoredMove> top = topScoredMoves(moves, hand, gstat, myId, kMcCandidateLimit);
    if (top.empty()) {
        return best;
    }

    best = top[0].move;
    int bestCombined = top[0].score / kMcBaseScoreDivisor
        + monteCarloScore(top[0].move, hand, seenCards, gstat, myId);
    for (size_t i = 1; i < top.size(); i++) {
        if (top[i].score + kMcOverrideWindow < top[0].score) {
            continue;
        }
        const int mcScore = monteCarloScore(top[i].move, hand, seenCards, gstat, myId);
        const int combined = top[i].score / kMcBaseScoreDivisor + mcScore;
        if (combined > bestCombined + kMcOverrideThreshold) {
...
bool SimplePlayer::follow(const GameStatus &gstat, CardSet &cs) {
    ...
    CandidateMove best = chooseBestMove(hand, memory, gstat, getId());
